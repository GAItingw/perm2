...
# null hypothesis is median old  = median new
test.diff <- median(grp.1) - median(grp.2)

it_median <- function(n){
  M = NULL
  for(i in 1:n){
    s = sample(data, lt, FALSE)
...
hist(examples, col = "red", breaks = 100, main="Random Permutations", xlab="")
abline(v = test.diff, col = "black", lwd = 4)
abline(v = quantile(examples, 0.025), col = "blue", lwd = 4)
abline(v = quantile(examples, 0.975), col = "orange", lwd = 4)

length(examples[examples<test.diff])/10000
# p-value = 0.4165 is too large
